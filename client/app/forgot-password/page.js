    ...
    <div className="relative flex min-h-screen items-center justify-center bg-paper px-4 py-12">
      ...
      <div className="w-full max-w-sm relative">
        ...
        <div className="rounded-2xl border border-line bg-card p-7 shadow-[0_16px_48px_rgba(0,0,0,0.6)]">
          {sent ? (
            <>
              <h1 className="font-display text-lg font-bold tracking-tight text-text">Check your email</h1>
              <p className="mt-2 text-sm leading-relaxed text-text-soft">
                If an account exists for <strong className="text-text">{email.trim()}</strong>, a password reset link is on its way. The link
                expires in an hour.
              </p>
              <Link href="/login" className="mt-6 inline-flex items-center justify-center w-full rounded-full bg-signal px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-signal-deep">
                Back to sign in
              </Link>
            </>
          ) : (
            <>
              <h1 className="font-display text-lg font-bold tracking-tight text-text">Reset your password</h1>
              <p className="mt-1.5 text-sm text-text-soft">Enter your email and we&apos;ll send you a link to choose a new password.</p>
              ...
              <Link href="/login" className="mt-4 block text-center text-sm text-text-soft hover:text-text">
                Back to sign in
              </Link>
            </>
          )}
