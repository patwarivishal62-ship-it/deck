...
export default function Breadcrumbs({ items }) {
  return (
    <nav aria-label="Breadcrumb" className="mb-5 flex items-center gap-1.5 font-mono text-xs text-[#7A8599]">
      {items.map((item, i) => {
        ...
        return (
          <span key={`${item.label}-${i}`} className="flex items-center gap-1.5">
            ...
            {isLast || !item.href ? (
              <span className={isLast ? "text-[#B8C0CC]" : ""} aria-current={isLast ? "page" : undefined}>
                {item.label}
              </span>
            ) : (
