...
export default function Breadcrumbs({ items }) {
  return (
    <nav aria-label="Breadcrumb" className="mb-4 flex items-center gap-1.5 font-mono text-xs text-text-faint">
      {items.map((item, i) => {
        ...
        return (
          <span key={`${item.label}-${i}`} className="flex items-center gap-1.5">
            ...
            {isLast || !item.href ? (
              ...
              <Link href={item.href} className="transition hover:text-signal-deep">
                {item.label}
              </Link>
            )}
          </span>
